...
class CliJoinServerReq : public CRosePacket {
    public:
        ...
        CliJoinServerReq& set_sessionId(const uint32_t);
        uint32_t get_sessionId() const;
        CliJoinServerReq& set_password(const Password&);
        const Password& get_password() const;
        
        
        static CliJoinServerReq create(const uint32_t& sessionId, const Password& password);
        static CliJoinServerReq create(const uint8_t* buffer);
        static std::unique_ptr<CliJoinServerReq> allocate(const uint8_t* buffer);
