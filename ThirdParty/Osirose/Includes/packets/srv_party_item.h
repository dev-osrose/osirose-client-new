...
class SrvPartyItem : public CRosePacket {
    public:
        static constexpr ePacketType PACKET_ID = ePacketType::PAKWC_PARTY_ITEM;
        SrvPartyItem();
        SrvPartyItem(CRoseReader reader);
        SrvPartyItem(SrvPartyItem&&) = default;
        SrvPartyItem& operator=(SrvPartyItem&&) = default;
        ~SrvPartyItem() = default;
        ...
        SrvPartyItem& set_target(const uint16_t);
        uint16_t get_target() const;
        SrvPartyItem& set_item(const Item);
        Item get_item() const;
        
        
        static SrvPartyItem create(const uint16_t& target, const Item& item);
        static SrvPartyItem create(const uint8_t* buffer);
        static std::unique_ptr<SrvPartyItem> allocate(const uint8_t* buffer);
