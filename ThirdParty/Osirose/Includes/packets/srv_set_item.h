...
class SrvSetItem : public CRosePacket {
    public:
        static constexpr ePacketType PACKET_ID = ePacketType::PAKWC_SET_ITEM;
        SrvSetItem();
        SrvSetItem(CRoseReader reader);
        SrvSetItem(SrvSetItem&&) = default;
        SrvSetItem& operator=(SrvSetItem&&) = default;
        ~SrvSetItem() = default;
        ...
        SrvSetItem& set_items(const std::vector<IndexAndItem>);
        SrvSetItem& add_items(const IndexAndItem);
        std::vector<IndexAndItem> get_items() const;
        IndexAndItem get_items(size_t index) const;
        
        
        static SrvSetItem create();
        static SrvSetItem create(const uint8_t* buffer);
        static std::unique_ptr<SrvSetItem> allocate(const uint8_t* buffer);
