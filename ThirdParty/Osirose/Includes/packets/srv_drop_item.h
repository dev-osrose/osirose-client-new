...
class SrvDropItem : public CRosePacket {
    public:
        static constexpr ePacketType PACKET_ID = ePacketType::PAKWC_DROP_ITEM;
        SrvDropItem();
        SrvDropItem(CRoseReader reader);
        SrvDropItem(SrvDropItem&&) = default;
        SrvDropItem& operator=(SrvDropItem&&) = default;
        ~SrvDropItem() = default;
        ...
        SrvDropItem& set_x(const float);
        float get_x() const;
        SrvDropItem& set_y(const float);
        float get_y() const;
        SrvDropItem& set_item(const Item);
        Item get_item() const;
        SrvDropItem& set_id(const uint16_t);
        uint16_t get_id() const;
        SrvDropItem& set_ownerId(const uint16_t);
        uint16_t get_ownerId() const;
        SrvDropItem& set_timeToLive(const uint16_t);
        uint16_t get_timeToLive() const;
        
        
        static SrvDropItem create();
        static SrvDropItem create(const uint8_t* buffer);
        static std::unique_ptr<SrvDropItem> allocate(const uint8_t* buffer);
