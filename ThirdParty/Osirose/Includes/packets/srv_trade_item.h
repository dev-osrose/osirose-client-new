...
class SrvTradeItem : public CRosePacket {
    public:
        static constexpr ePacketType PACKET_ID = ePacketType::PAKWC_TRADE_ITEM;
        SrvTradeItem();
        SrvTradeItem(CRoseReader reader);
        SrvTradeItem(SrvTradeItem&&) = default;
        SrvTradeItem& operator=(SrvTradeItem&&) = default;
        ~SrvTradeItem() = default;
        ...
        SrvTradeItem& set_slot(const uint8_t);
        uint8_t get_slot() const;
        SrvTradeItem& set_item(const Item);
        Item get_item() const;
        
        
        static SrvTradeItem create(const uint8_t& slot, const Item& item);
        static SrvTradeItem create(const uint8_t* buffer);
        static std::unique_ptr<SrvTradeItem> allocate(const uint8_t* buffer);
